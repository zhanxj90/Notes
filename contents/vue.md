...
## mixin
### *mixin释义*
  1. mixins (混入) 是一种分发 Vue 组件中可复用功能的非常灵活的方式。
  2. 混入对象可以包含任意组件选项。
  3. 当组件使用混入对象时，所有混合对象的选项将被混入该组件本身的选项。
### *mixin和组件的区别*
  1. 组件相当于在父组件内开辟了一块单独的空间，来根据父组件props过来的值进行相应的操作，单本质上两者还是泾渭分明，相对独立。
  2. mixin则是将父组件里的内容和mixin的合并，扩充父组件内容，mixin就相当于基础数据

-----

## 模板
### *模板的本质*
  1. 模板的本质是渲染函数的语法糖，描述了界面的渲染过程，最终产生表达界面结构的虚拟节点
### *模板编译*
  1. 模板转换成渲染函数的过程叫模板编译
  2. 编译时机
      - 运行时编译：浏览器运行时编译，会损失效率；页面中引入vue的cdn使用的是这种编译
      - 预编译：编译时态的编译（打包时态的编译），vue工程化开发时使用的
