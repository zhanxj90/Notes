...
## 网络状态监控
### *网络状态api*
  1. navigator.connection：获取网络状态的浏览器api，返回对象，带有网络状态、带宽等属性
      - downlink：用户下载速度，即带宽，单位兆比特每秒（Mb/s）
      - rtt：数据包发送和返回的时间，即带宽延迟，单位毫秒（ms）
      - effectiveType：用户的网络类型（2g，3g，4g）；不一定是真实的类型，是通过带宽和延迟计算出来的，即wif可能是3g也可能是4g
  2. navigator.onLine：指示是不是网络在线，true-在线，false-离线
### *网络状态事件*
  1. 监听网络是否在线
      ```js
        // 在线
  ...
  2. 监听网速的切换
      ...
      ```
  3. 更加细致的监听，如带宽和延迟的监听：没有事件监听，可自定义定时器监听